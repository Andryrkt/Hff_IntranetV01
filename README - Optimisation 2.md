...
echo "✅ Conteneur : {$cacheDir}/Container.php\n";
...
**Rôle** : Charger le conteneur et adapter le comportement selon l'environnement (DEV/PROD). Précompiler les templates Twig au premier démarrage en PROD.
        ...
        if (is_dir($templateDir)) {
            ...
            foreach ($iterator as $file) {
                ...
                try {
                    ...
                    // C'est normal en précompilation, on compile juste la structure
                    $compiledCount++;
                    if ($isDebug) {
                        error_log("  ⚠️  RuntimeError (OK) {$templateName}: {$e->getMessage()}");
                    }
                } catch (\Exception $e) {
                    // Autre erreur
                    $errorCount++;
...
- **En DEV (`$isDebug = true`)** : `isFresh()` vérifie les dates de modification des fichiers sources
- **En PROD (`$isDebug = false`)** : `isFresh()` retourne toujours `true` (pas de vérification)
...
echo "🔨 Compilation du conteneur et des routes..."
php config/bootstrap_build.php

echo "📦 Nettoyage des fichiers temporaires..."
rm -rf var/cache/dev/*

echo "⚙️ Configuration environnement PROD..."
echo "APP_ENV=prod" > .env
echo "APP_DEBUG=false" >> .env

echo "✅ Déploiement terminé"
echo "💡 Les templates Twig seront compilés au premier démarrage"
```
...
if (file_exists($containerCache)) {
    ...
    echo "✅ Cache conteneur supprimé\n";
}

echo "\n🎉 Cache nettoyé avec succès\n";
echo "💡 Relancez : php config/bootstrap_build.php\n";
```
...
# .env.local (non versionné, ignoré par Git)
APP_ENV=dev
APP_DEBUG=true
```
...
### 5. Vérifier les Statistiques de Compilation Twig
...
**En DEV** :
- Vérifier que `APP_DEBUG=true` dans `.env`
