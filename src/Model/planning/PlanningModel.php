...
class PlanningModel extends Model
{
    ...
    public function recupMatListeTous($criteria, string $lesOrValides, string $back, $touslesOrSoumis)
    {
        ...
        $statement = "SELECT 
                ...
                trim(mmat_recalph)            AS numParc,
                trim(mmat_numparc)            AS casier,
                {$this->planAnnee($criteria)} AS annee,
                {$this->planMonth($criteria)} AS mois,
                seor_numor ||'-'||sitv_interv AS orIntv,
                (SELECT 
                    SUM( 
                        CASE WHEN slor_typlig = 'P' $vligneType  THEN
                            ROUND(slor_qterel + slor_qterea + slor_qteres + slor_qtewait - slor_qrec)
                        ELSE ROUND(slor_qterea) 
                        END 
                    )
                ...
                CASE 
                    WHEN slor_typlig = 'P' 
                    THEN ROUND( (slor_qterel + slor_qterea + slor_qteres + slor_qtewait - slor_qrec) )
                    ELSE ROUND( slor_qterea )
                END AS QteRes_Or,
            ...
            WHERE seor_numor = slor_numor
                ...
                $vOrvalDw
                {$this->facture($criteria)}     
                {$this->typeLigne($criteria)}
                {$this->nonplannfierSansDatePla($criteria)}
                {$this->agence($criteria)}
                {$this->interneExterne($criteria)}
                {$this->agenceDebite($criteria)}
                {$this->serviceDebite($criteria)}
                {$this->dateDebutMonthPlan($criteria)}
                {$this->dateFinMonthPlan($criteria)}
                {$this->numParc($criteria)}
                {$this->idMat($criteria)}
                {$this->numOr($criteria)}
                {$this->numSerie($criteria)}
                {$this->casier($criteria)}
