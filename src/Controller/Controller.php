        ...
        if ($this->sessionService === null) {
            try {
                ...
                if ($container && $container->has('App\\Service\\SessionManagerService')) {
                    ...
                    // Fallback: créer SessionManagerService avec la session du container
                    $session = $container ? $container->get('session') : null;
                    $this->sessionService = new \App\Service\SessionManagerService($session);
                }
            } catch (\Throwable $e) {
                // Fallback: créer SessionManagerService avec la session du container
                $container = $this->getContainer();
