...
class DaAffectationAchatController extends Controller
{
    ...
    public function affectationDaAchat($id, Request $request)
    {
        ...
        $form = $this->getFormFactory()->createBuilder(DemandeApproParentType::class, $daParent)->getForm();

        dump($daParent);
        foreach ($daParent->getDemandeApproParentLines() as $demandeApproParentLine) {
            dump($demandeApproParentLine);
        }
        die;
