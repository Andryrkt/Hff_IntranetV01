...
class DitOrsSoumisAValidationController extends BaseController
{
    ...
    public function insertionOr(Request $request, $numDit)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($this->bloquageOrSoumsi($conditionBloquage, $originalName, $ditInsertionOrSoumis)) {
                ...
                $orSoumisValidationModel = $this->ditModelService->recupOrSoumisValidation($ditInsertionOrSoumis->getNumeroOR());
                ...
                $this->envoiePieceJoint($form, $ditInsertionOrSoumis, $this->fusionPdfService, $suffix, $mainPdf);

                //fusion de pdf Demande appro avec le pdf OR fusionner
                // $this->fusionPdfServiceDaAvecORfusionner($numDit, $mainPdf);
    ...
    private function fusionPdfDaAvecORfusionner(string $numDit, string $mainPdf): void
    {
        ...
        if (!empty($daAfficherValiders)) {
            ...
            //fusion des fichiers
            $this->fusionPdfService->mergePdfs($pdfFiles, $mainPdf);
        }
    }
    ...
    private function conditionsDeBloquegeSoumissionOr(string $originalName, string $numOr, $ditInsertionOrSoumis, string $numDit): array
    {
        ...
        $agServInformix = $this->ditModelService->recupAgenceServiceDebiteur($ditInsertionOrSoumis->getNumeroOR());
    ...
    {
        $numDevis = $this->ditModelService->recupererNumdevis($numOr);
