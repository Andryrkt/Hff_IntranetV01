$(document).ready(function () {
  ...
  function handleServINtChange() {
    ...
    if (typeMission === 'MUTATION' && codeServ === '50') {
      $.ajax({
        type: 'POST',
        url: '/Hffintranet_TEST/selectCatgeRental',
        data: {
          CodeRental: codeServ,
        },
  ...
  function handleSiteRental() {
    ...
    if (MutaRental.trim() !== '') {
      if (typeMission === 'MUTATION' && codeServ === '50') {
        $.ajax({
          type: 'POST',
          url: '/Hffintranet_TEST/selectIdem',
          data: {
  ...
  function handlePrixRental() {
    ...
    if (SiteRental01.trim() !== '') {
      if (typeMission === 'MUTATION' && codeServ === '50') {
        $.ajax({
          type: 'POST',
          url: '/Hffintranet_TEST/selectPrixRental',
          data: {
            typeMiss: typeMission,
            categ: MutaRental,
...
function selectAgenceInterne() {
  ...
  let url = `/Hffintranet_TEST/serviceDebiteur-fetch/${agenceDebiteur}`;
  fetch(url)
