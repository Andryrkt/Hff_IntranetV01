...
function sommeEtIndemnite(champA, champB, champC, champ2) {
  ...
  let valeurChampA =
    parseFloat(document.getElementById(champA).value.replace(/[^\d]/g, "")) ||
    0;
  let valeurChampC =
    parseFloat(document.getElementById(champC).value.replace(/[^\d]/g, "")) ||
    0;
...
function Somme() {
  ...
  var SmontIndemTotal = parseFloat(
    montIndemTotal.replace(/\./g, "").replace(",", "")
  );
  if (mont01 === "") {
    Smont01 = 0;
  }
  ...
  STotalAutre = parseFloat(
    TotalAutre.value.replace(/\./g, "").replace(",", ".")
  );
...
function calculerSomme(champA, champB, champC, TotalC) {
  // Récupérer les valeurs des deux champs
  let valeurChampA =
    parseFloat(document.getElementById(champA).value.replace(".", "")) || 0;
  let valeurChampB =
    parseFloat(document.getElementById(champB).value.replace(".", "")) || 0;
  let valeurChampC =
    parseFloat(document.getElementById(champC).value.replace(".", "")) || 0;
  // Calculer la somme
...
function negative(TotalAll) {
  let valeur_TotalAll =
    parseFloat(document.getElementById(TotalAll).value.replace(".", "")) || 0;
